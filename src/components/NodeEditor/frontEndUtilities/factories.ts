import type { XYPosition } from '@xyflow/react';
import type {
  ColumnConfig,
  FinanceFlowNode,
  FinanceNodeType,
  FinanceWidgetKind,
  ParentTableNodeData,
  WidgetNodeData,
  WorkflowGraph,
} from './types';

let idCounter = 0;

function nextId(prefix: string): string {
  idCounter += 1;
  return `${prefix}-${Date.now()}-${idCounter}`;
}

function baseNode<TType extends FinanceNodeType, TData extends FinanceFlowNode['data']>(
  id: string,
  type: TType,
  position: XYPosition,
  data: TData,
  extras?: Partial<FinanceFlowNode>
): FinanceFlowNode {
  return {
    id,
    type,
    position,
    data,
    ...extras,
  } as FinanceFlowNode;
}

export function createStartNode(position: XYPosition = { x: 160, y: 240 }): FinanceFlowNode {
  return baseNode(
    'start-root',
    'start',
    position,
    {
      kind: 'start',
      label: 'Start',
      subtitle: 'Begin workflow execution',
      icon: '▶',
      accentColor: '#00d18f',
    },
    { draggable: false }
  );
}

export function createParentTableNode(position: XYPosition = { x: 420, y: 180 }): FinanceFlowNode {
  const defaultColumns: ColumnConfig[] = [
    { id: 'col-budget',    header: 'Budget',    subheader: 'Planned',  headerColor: '#3b82f6', subheaderColor: '#93c5fd' },
    { id: 'col-actual',    header: 'Actual',    subheader: 'Spent',    headerColor: '#3b82f6', subheaderColor: '#93c5fd' },
    { id: 'col-diff',      header: 'Difference', subheader: 'Variance', headerColor: '#3b82f6', subheaderColor: '#93c5fd' },
  ];

  const data: ParentTableNodeData = {
    kind: 'parentTable',
    label: 'Parent Table',
    subtitle: 'Spreadsheet with auto-generated child tables',
    icon: 'PT',
    accentColor: '#3b82f6',
    description: 'Parent-child budget table generator',
    columns: defaultColumns,
    rows: [],
  };

  return baseNode(nextId('parentTable'), 'parentTable', position, data, { width: 700, height: 320 });
}

function createWidgetData(type: FinanceWidgetKind): WidgetNodeData {
  switch (type) {
    case 'verticalBarChart':
      return {
        kind: 'widget',
        widgetType: type,
        category: 'charts',
        label: 'Vertical Bar Chart',
        subtitle: 'Category comparison view',
        icon: 'VB',
        accentColor: '#f59e0b',
        metrics: ['Needs', 'Wants', 'Savings'],
        previewKind: 'barV',
      };
    case 'horizontalBarChart':
      return {
        kind: 'widget',
        widgetType: type,
        category: 'charts',
        label: 'Horizontal Bar Chart',
        subtitle: 'Ranked spend breakdown',
        icon: 'HB',
        accentColor: '#f59e0b',
        metrics: ['Top categories', 'This month'],
        previewKind: 'barH',
      };
    case 'pieChart':
      return {
        kind: 'widget',
        widgetType: type,
        category: 'charts',
        label: 'Pie Chart',
        subtitle: 'Budget distribution',
        icon: 'PI',
        accentColor: '#f59e0b',
        metrics: ['Needs 42%', 'Wants 28%', 'Savings 30%'],
        previewKind: 'pie',
      };
    case 'trendLine':
      return {
        kind: 'widget',
        widgetType: type,
        category: 'charts',
        label: 'Trend Line',
        subtitle: 'Monthly movement over time',
        icon: 'TR',
        accentColor: '#f59e0b',
        metrics: ['12 months', 'Rolling avg'],
        previewKind: 'line',
      };
    case 'calculate':
      return {
        kind: 'widget',
        widgetType: type,
        category: 'operations',
        label: 'Calculate',
        subtitle: 'Formula and derived values',
        icon: '∑',
        accentColor: '#6366f1',
        metrics: ['Net = Income - Expense', 'Margin %'],
        previewKind: 'operation',
      };
    case 'filter':
      return {
        kind: 'widget',
        widgetType: type,
        category: 'operations',
        label: 'Filter',
        subtitle: 'Conditions and row pruning',
        icon: 'FL',
        accentColor: '#6366f1',
        metrics: ['Month = Current', 'Bucket != Surplus'],
        previewKind: 'operation',
      };
    case 'income':
      return {
        kind: 'widget',
        widgetType: type,
        category: 'operations',
        label: 'Income',
        subtitle: 'Track inflows and salary',
        icon: '$',
        accentColor: '#6366f1',
        metrics: ['Monthly $7,812', '2 sources'],
        previewKind: 'operation',
      };
    case 'expense':
      return {
        kind: 'widget',
        widgetType: type,
        category: 'operations',
        label: 'Expense',
        subtitle: 'Track outflows and categories',
        icon: '−',
        accentColor: '#6366f1',
        metrics: ['Monthly $5,924', '38 categories'],
        previewKind: 'operation',
      };
    case 'balance':
      return {
        kind: 'widget',
        widgetType: type,
        category: 'operations',
        label: 'Balance',
        subtitle: 'Cash flow and net position',
        icon: '≈',
        accentColor: '#6366f1',
        metrics: ['Net +$1,888', 'Runway 4.2 mo'],
        previewKind: 'operation',
      };
    default:
      return {
        kind: 'widget',
        widgetType: type,
        category: 'operations',
        label: type,
        subtitle: 'Workflow node',
        icon: '•',
        accentColor: '#94a3b8',
        metrics: [],
      };
  }
}

export function createWidgetNode(type: FinanceWidgetKind, position: XYPosition): FinanceFlowNode {
  return baseNode(nextId(type), type, position, createWidgetData(type));
}

export function createNodeFromLibrary(type: FinanceNodeType, position: XYPosition): FinanceFlowNode | null {
  if (type === 'start') return createStartNode(position);
  if (type === 'parentTable') return createParentTableNode(position);

  return createWidgetNode(type, position);
}

export function createBlankWorkflowGraph(): WorkflowGraph {
  return {
    nodes: [createStartNode(), createParentTableNode()],
    edges: [],
  };
}

export function createEmptyWorkflowGraph(): WorkflowGraph {
  return {
    nodes: [],
    edges: [],
  };
}
